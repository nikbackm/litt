...
**********************************************************************************************************************/
// check lengtht and format of DR when entered!
...
public:
	...
	})
	{
		for (int i = 1; i < argc; ++i) {
			if (argv[i][0] == '-' && argv[i][1] != '\0') { // A stand-alone '-' can be used as an (action) argument.
				...
				if (m_action.empty()) {
					m_action = argv[i];
					if (m_action.length() >= 2 && m_action[0] == WildCard) {
						m_actionLeftWildCard = WildCardStr;
						m_action.erase(0, 1);
					}
					if (m_action.length() >= 2 && m_action.back() == WildCard) {
						m_actionRightWildCard = WildCardStr;
						m_action.pop_back();
					}
				}
				else {
	...
	std::vector<std::string> selectRowValue(std::string const& userSql)
	{
		...
		{
			static_cast<decltype(&res)>(pArg)->assign(argv, argv + argc);
			return 0;
		};
	...
	std::string selDV(std::string const& sql, const char* idName)
	{
	...
	void setBookGenre(IdValue bookId, std::string const& gcmd, IdValue genreId)
	{
		...
		auto const genre = (gcmd == "c" || gcmd == "d") ? selGenre(genreId) : "";
		int changes = -1;
