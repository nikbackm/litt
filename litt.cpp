...
struct LittState {
	...
	std::vector<ColumnInfo*> columnOrder; // Overrides the default action order.
	...
	int rowCount = 0; // The number of rows printed so far.
	...
	for (int i = 1; i < argc; ++i) {
		if (argv[i][0] == '-' && argv[i][1] != '\0') { // NOTE: We will allow a single '-' to be used as an action argument.
			...
			case 'h':
				...
				break;
			case 'o': 
				ls.columnOrder = ls.getColumns(optVal);
				if (ls.columnOrder.empty()) { return false; }
				break;
			case 'c': 
...
void restoreOutputSettingsIfConsole(LittState ls)
{
	if (ls.stdOutIsConsole) {
...
int runSqlOutputQuery(std::string const & sql, LittState ls)  
{
	if (ls.showQuery) {
	...
	auto callback = [](void *pArg, int argc, char **argv, char **azColName) {
		...
		auto ls = static_cast<LittState*>(pArg);
		if (ls->rowCount++ == 0) {
			for (int i = 0; i < argc; i++) {
				//printf("%s", azColName[i]);
				auto s = utf8ToWide(azColName[i]);
				WriteConsole(std_out, s.c_str(), s.length(), &dw, 0);
				//if (i + 1 != argc) putchar('|');
			}
			//printf("\n");
			auto s = utf8ToWide("\n");
		...
		for (int i = 0; i < argc; i++) {
			...
			auto s = utf8ToWide(argv[i] ? argv[i] : "");
			WriteConsole(std_out, s.c_str(), s.length(), &dw, 0);
			//if (i + 1 != argc) putchar('|');
		}
		//printf("\n");
	...
	res = sqlite3_exec(db, sql.c_str(), callback, &ls, &zErrMsg);
	if (res != SQLITE_OK) {
...
ORDER BY \"Date read\", Books.\"BookID\", \"Last Name\", \"First Name\", \"Title\";";
	...
	return runSqlOutputQuery(sqlb, ls);
}
...
int main(int argc, char **argv)
{
	...
	//auto str = "esldfdlkf dslkfjdslkfjsdlk sdflkjflkdsjf sdljfds\n";
	//auto p = buf;

	//setvbuf(stdout, buf2, _IOFBF, sizeof buf2);

	//for (int i = 0; i < 20000; ++i)
	//{
		//printf("lastname%i firstname%i title%i\n", i, i, i);

		//fprintf(stdout, "lastname%i firstname%i title%i\n", i, i, i);

		//p += sprintf(p, "lastname%i firstname%i title%i\n", i, i, i);

		//sprintf_s(buf, "lastname%i firstname%i title%i\n", i, i, i);
 ...
 //   return 0;
	...
	char *sqlbrym = 
		"SELECT Main.\"Year\" AS \"Year\", Total, \"Jan\", \"Feb\", \"Mar\", \"Apr\", \"May\", \"Jun\", \"Jul\", \"Aug\", \"Sep\", \"Oct\", \"Nov\", \"Dec\" FROM\
	(SELECT \"Year\", Count(BookID) as Total FROM\
	   (SELECT BookID, strftime('%Y', \"Date Read\") AS \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE \"Date Read\" > \"2002\")\
	 GROUP BY \"Year\"\
	 ) Main\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Jan\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-01-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Feb\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-02-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Mar\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-03-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Apr\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-04-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"May\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-05-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Jun\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-06-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Jul\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-07-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Aug\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-08-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Sep\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-09-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Oct\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-10-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Nov\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-11-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	LEFT OUTER JOIN\
	(SELECT \"Year\", Count(BookID) AS \"Dec\" FROM\
	   (SELECT Books.BookID, strftime('%Y', \"Date Read\") as \"Year\"\
		FROM Books\
		INNER JOIN DatesRead USING(BookID)\
		WHERE (\"Date Read\" > \"2002\") AND ((\"Date read\" LIKE '%-12-%')))\
	 GROUP BY \"Year\")\
	USING(\"Year\")\
	ORDER BY \"Year\";";
